...
export const isComment = (v: any): v is Comment => {
  return isDOMNode(v) && v.nodeType === TYPE_COMMENT_NODE;
};
 ...
 */
export const isDOMNode = (v: any): v is Node => isObj(v) && "nodeType" in v;
...
export const isHTMLElement = (v: any): v is HTMLElement => {
  return isDOMNode(v) && v.nodeType === TYPE_ELEMENT;
};
...
export const isTextNode = (v: any): v is Text => {
  return isObj(v) && "nodeType" in v && v.nodeType === TYPE_TEXT_NODE;
};
